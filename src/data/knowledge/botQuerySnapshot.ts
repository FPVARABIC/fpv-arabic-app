 ...
 *
 * How to use:
 *  - `nluIntent` is the output of classifyBotIntent() traced from INTENT_RULES.
 *  - `isFpv` is the output of detectFpvDomain().
 *  - `interceptFires` names the intercept in buildKnowledgeAnswer (A–H) that
 *    handles this query, or "core" if it falls through to _buildKnowledgeAnswerCore.
 *  - `expectedAnswerMode` is the logical mode the current code enters in core.
 *  - `knownIssue` documents confirmed bugs that future phases must fix.
 *
...
export interface BotQuerySnapshot {
  ...
  category:
    ...
    | 'navigation';
  query: string;
  // ── NLU outputs (classifyBotIntent + detectFpvDomain) ──────────────────────
  nluIntent: string;
  isFpv: boolean;
  // ── Pipeline routing ───────────────────────────────────────────────────────
...
/**
 * Known-issue summary across the 50 baseline queries.
 * Tracked here for Phase -1 reporting; addressed in later phases.
 */
