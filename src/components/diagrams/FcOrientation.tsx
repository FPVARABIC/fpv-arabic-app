    ...
    <DiagramFrame title="تركيب Flight Controller" hint="اضغط أي جزء لمعرفة دوره">
      <svg viewBox="0 0 220 180" className="w-full">
        ...
        {[[55, 55], [165, 55], [55, 125], [165, 125]].map(([x, y], i) => (
          <circle key={i} cx={x} cy={y} r="10" fill="none" stroke={sel === 'grommet' ? C.cyan : 'rgba(148,163,184,0.6)'} strokeWidth={sel === 'grommet' ? 3 : 2} onClick={() => handleToggle('grommet')} style={{ cursor: 'pointer' }} data-testid="fc-orientation-part-grommet" />
        ))}
        {/* board */}
        ...
        {/* forward arrow */}
        <g onClick={() => handleToggle('arrow')} style={{ cursor: 'pointer' }} data-testid="fc-orientation-part-arrow">
        ...
        {/* usb */}
        <rect x="60" y="130" width="22" height="10" rx="2" fill={sel === 'usb' ? C.cyan : 'rgba(148,163,184,0.5)'} onClick={() => handleToggle('usb')} style={{ cursor: 'pointer' }} data-testid="fc-orientation-part-usb" />
