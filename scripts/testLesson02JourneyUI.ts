...
async function main() {
  ...
  try {
    ...
    // ── Regression: Lesson 1 still uses its own journey; Lesson 10 diagram intact ──
    {
    ...
    {
      // Lessons 3, 4, 5, 6, 7, 8, and 9 were deliberately migrated onto the
      // journey architecture in Phases 5-11 (see testLesson03JourneyUI.ts /
      // testLesson04JourneyUI.ts / testLesson05JourneyUI.ts /
      // testLesson06JourneyUI.ts / testLesson07JourneyUI.ts /
      // testLesson08JourneyUI.ts / testLesson09JourneyUI.ts) — Lesson 10 is
    ...
    }
    {
      ...
      await page.goto(`${BASE}/lessons/lesson-pre-battery-safety`, { waitUntil: 'networkidle' });
      await page.waitForTimeout(300);
      ok('Lesson 10 (pre-battery-safety) still renders its existing interactive diagram (no regression from the SignalFlow prop addition)', await page.locator('svg').count() > 0);
