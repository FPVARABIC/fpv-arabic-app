...
async function main() {
  ...
  try {
    ...
    // ── Regression: Lesson 1 still uses its own journey; Lesson 9 diagram intact ──
    {
    ...
    {
      // Lessons 3, 4, and 5 were deliberately migrated onto the journey
      // architecture in Phases 5-7 (see testLesson03JourneyUI.ts /
      // testLesson04JourneyUI.ts / testLesson05JourneyUI.ts) — Lesson 6 is
      ...
      await page.waitForTimeout(300);
      ok('Lesson 6 still uses the generic legacy lesson page (no journey stage rendered)', await page.locator('[data-testid="lesson01-stage"]').count() === 0);
    ...
    }
    {
      ...
      await page.waitForTimeout(300);
      ok('Lesson 9 (tx-rx) still renders its existing interactive diagram (no regression from the SignalFlow prop addition)', await page.locator('svg').count() > 0);
