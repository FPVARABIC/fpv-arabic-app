 ...
 *   4. «المجتمع» IS OFF THE PRIMARY NAVIGATION AND NOTHING MORE — no tab, no
 *      home card, and every route, page, site-map entry and backend adapter
 *      still in place. Section [8] asserts both halves; the second is what
 *      makes putting it back a one-line change.
 *
 * WHERE THIS SUITE CAME FROM
 * --------------------------
 * The section it covers was written on `claude/web-platform-foundation` and
 * ported here whole rather than rewritten — porting is the reason the platform
 * has one build path and not two. Two of its assertions had to be re-aimed on
 * arrival, and each says so at the point of change: [1] no longer pins the tab
 * ORDER (`scripts/testNavOrder.ts` owns that now), and [8] no longer asserts
 * the community was retired, because on this line of work it was not.
 *
...
// ─────────────────────────────────────────────────────────────────────────────
console.log('\n[1] «البناء» is reachable from the bar and from the site map');
{
  /*
   * THE ORDER IS NOT DECIDED HERE ANY MORE.
   *
   * This suite arrived with the bar it was written against — home · البناء ·
   * البرامج · الموسوعة · المشاريع · المتجر — pinned as a literal. The owner has
   * since fixed a different order, and `scripts/testNavOrder.ts` is the single
   * place that owns it. Two suites pinning two orders is how a merge silently
   * reverts a decision, so this one keeps only what it is actually about:
   * «البناء» is ON the bar, it points at its own route, and the site map knows
   * about it. Where it sits in the sequence is the other suite's question.
   *
   * The community assertions changed for the same reason. On the branch this
   * came from, «المجتمع» had been retired from the site map entirely. Here it
   * has NOT been retired — it is off the tab bar and nothing else, by explicit
   * instruction («لا تحذف كود المجتمع أو بياناته أو Routes الخاصة به»). So the
   * tab assertion stands and the site-map assertion is inverted: the entry must
   * still be there.
   */
  const buildTab = NAV_TABS.find(t => t.id === 'build');
  ok('«البناء» is on the tab bar', !!buildTab);
  ok('…and points at its own route', buildTab?.href === '/build' && buildTab?.labelAr === 'البناء');
  ...
  ok('…and the community entry is still there, because it was never deleted',
    NAV_ITEMS.some(i => i.id === 'community' && i.href === '/community'));
  ok('the sitemap therefore lists /build by construction',
...
// ─────────────────────────────────────────────────────────────────────────────
console.log('\n[8] «المجتمع» is off the primary navigation — and nothing more');
{
  /*
   * THIS SECTION WAS INVERTED WHEN THE BUILD SECTION WAS PORTED HERE.
   *
   * It arrived asserting that the community had been RETIRED: struck from the
   * site map, dropped from search, and redirected wholesale to /build. That was
   * the decision on the branch this section came from. It is not the decision
   * here, and asserting it would have quietly deleted a section this line of
   * work was told to keep — «لا تحذف كود المجتمع أو بياناته أو Routes الخاصة
   * به. فقط أخفِه من التنقل الرئيسي والصفحة الرئيسية».
   *
   * So the assertions now hold the instruction as written: off the bar, off the
   * home cards, everything else untouched and still working. Both halves
   * matter — the second is what makes putting it back a one-line change.
   */
  ok('no tab points at the community', NAV_TABS.every(t => t.href !== '/community'));
  ok('the home page has no community card',
    !read('web/app/page.tsx').includes("id: 'community'"));

  // And NOT removed:
  ok('the site map still carries the community entry',
    NAV_ITEMS.some(i => i.id === 'community' && i.href === '/community'));
  ok('the community route still exists',
  ...
  ok('search still queries the community backend',
    read('web/app/search/page.tsx').includes('searchCommunity'));
  ok('nothing redirects /community away',
    !/source: '\/community'/.test(read('web/next.config.ts')));
