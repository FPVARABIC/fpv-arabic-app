...
async function main() {
  ...
  try {
    ...
    // ── Regression: Lesson 1 & 2 still use their own journeys; Lesson 13 legacy intact ──
    {
    ...
    {
      // Lessons 4, 5, 6, 7, 8, 9, 10, 11, and 12 were deliberately migrated
      // onto the journey architecture in Phases 6-14 (see testLesson04JourneyUI.ts /
    ...
    }
    {
      ...
      await page.goto(`${BASE}/lessons/lesson-esc-install`, { waitUntil: 'networkidle' });
      await page.waitForTimeout(300);
      ok('Lesson 13 (esc-install) still renders its hero image (it has an image field, so no SVG diagram is expected; no regression from the PartsMap prop addition)', await page.locator('img').count() > 0);
