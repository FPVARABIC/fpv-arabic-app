...
async function main() {
  ...
  try {
    ...
    // ── Regression: Lessons 1-3 still use their own journeys; Lesson 15 legacy intact ──
    {
    ...
    {
      // Lessons 5, 6, 7, 8, 9, 10, 11, 12, 13, and 14 were deliberately migrated
      // onto the journey architecture in Phases 7-16 (see testLesson05JourneyUI.ts /
      // testLesson06JourneyUI.ts / testLesson07JourneyUI.ts /
      // testLesson08JourneyUI.ts / testLesson09JourneyUI.ts /
      // testLesson10JourneyUI.ts / testLesson11JourneyUI.ts /
      // testLesson12JourneyUI.ts / testLesson13JourneyUI.ts /
      // testLesson14JourneyUI.ts) — Lesson 15 is now the nearest still-legacy
    ...
    }
    {
      ...
      await page.goto(`${BASE}/lessons/lesson-receiver-install`, { waitUntil: 'networkidle' });
      await page.waitForTimeout(300);
      ok('Lesson 15 (receiver-install) still renders its hero image (it has an image field, so no SVG diagram is expected; no regression from this phase)', await page.locator('img').count() > 0);
      const overflow9 = await page.evaluate(() => document.documentElement.scrollWidth > document.documentElement.clientWidth + 1);
      ok('no horizontal overflow on Lesson 15 either', !overflow9);
      await ctx.close();
    }
    {
      const ctx = await browser.newContext({ viewport: { width: 390, height: 844 } });
