...
async function main() {
  ...
  try {
    ...
    // ── Scenario D: Lesson 5 still renders generically (Lessons 2, 3, and 4
    // were deliberately migrated onto the journey architecture in Phases 4-6
    // — see testLesson02JourneyUI.ts / testLesson03JourneyUI.ts /
    // testLesson04JourneyUI.ts — so Lesson 5 is now the nearest still-legacy
    ...
    // only diagram not shadowed by an image) still works ────────────────────
    {
      ...
      await page.waitForTimeout(400);
      ok('Lesson 5 does NOT use the stage-based journey UI', await page.locator('[data-testid="lesson01-stage"]').count() === 0);
      ok('Lesson 5 still shows the generic "الشرح" explanation heading', await page.locator('text=الشرح').count() === 1);
      ok('Lesson 5 still shows its level/duration pill-stat badges', await page.locator('.pill-stat').count() === 2);
