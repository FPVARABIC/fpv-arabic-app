...
async function main() {
  ...
  try {
    ...
    // ── Scenario D: Lessons 17 and 18 (and their containing section) were
    // removed from the app entirely (Lessons 2-16 were deliberately migrated
    ...
    // testLesson15JourneyUI.ts / testLesson16JourneyUI.ts — Lesson 16 is now
    // the final lesson in the app) ──────────────────────────────────────────
    {
      const page = await browser.newPage({ viewport: { width: 390, height: 844 } });
      page.on('console', m => { if (m.type() === 'error') consoleErrors.push(m.text()); });
      page.on('pageerror', e => consoleErrors.push(String(e)));
