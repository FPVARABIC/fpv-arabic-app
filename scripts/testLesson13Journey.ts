/**
 * Real assertions against Lesson 13's journey definition
 * (src/data/lessons/lesson13Journey.definition.ts) driven through the
 ...
 * scripts/testLesson13JourneyUI.ts proves the same behaviors are actually
...
import { lesson13JourneyDefinition as def } from '../src/data/lessons/lesson13Journey.definition';
...
console.log('\n[1] Registration: Lesson 13 is registered in the journey registry with real lesson data');
{
  const lesson13 = lessonsData.find(l => l.id === 'lesson-esc-install')!;
  ok('lesson13JourneyDefinition.lessonId matches the real Lesson 13 id', def.lessonId === lesson13.id);
  ok('getLessonJourneyDefinition resolves Lesson 13 to this exact definition', getLessonJourneyDefinition(lesson13.id) === def);
  ok('Lesson 13 has 17 stages', STAGE_COUNT === 17);
}

console.log('\n[2] The esc-placement diagram stage exists with exactly the 3 real part ids as required variants');
{
  ok('exactly one interactive_diagram stage exists', def.stages.filter(s => s.type === 'interactive_diagram').length === 1);
  ok('diagramType is esc-placement', DIAGRAM_STAGE.diagramType === 'esc-placement');
  ok('required variants are exactly the 3 real parts', JSON.stringify(DIAGRAM_STAGE.requiredVariants) === JSON.stringify(['esc', 'signal', 'power']));
}
...
console.log('\n[5] ESC-placement diagram completion requires exploring all 3 parts');
{
  let s = createInitialSessionState(def);
  ok('not complete before any part explored', !isInteractionComplete(DIAGRAM_STAGE, s));
  s = recordInteractionVariant(s, DIAGRAM_STAGE.id, 'esc');
  ok('still not complete after exploring only "esc"', !isInteractionComplete(DIAGRAM_STAGE, s));
  s = recordInteractionVariant(s, DIAGRAM_STAGE.id, 'esc'); // repeat, should not substitute for the others
  ok('repeatedly exploring the same part never substitutes for exploring the others', !isInteractionComplete(DIAGRAM_STAGE, s));
  s = recordInteractionVariant(s, DIAGRAM_STAGE.id, 'signal');
  ok('still not complete after exploring 2 of 3 parts', !isInteractionComplete(DIAGRAM_STAGE, s));
  s = recordInteractionVariant(s, DIAGRAM_STAGE.id, 'power');
...
console.log('\n[7] All four required concept checks exist and each has exactly one correct option with feedback');
{
  ...
  ok('checkpoint ids are: whyLocationMatters, coolingNotRacingOnly, securePlacementPrinciple, routingAndPlacementReasoning',
    JSON.stringify(ids) === JSON.stringify([
      'whyLocationMatters', 'coolingNotRacingOnly', 'securePlacementPrinciple', 'routingAndPlacementReasoning',
    ]));
}
...
console.log('\n[9] Completion remains unavailable until every checkpoint, the diagram, and recall have been engaged with');
{
  ...
  ok('still not ready — ESC-placement diagram not yet explored', !isReadyToComplete(def, s));
...
console.log('\n[10] Readiness checklist always names exactly what remains, in the declared order');
{
  ...
  ok('requirements are returned in the declared readinessOrder (first checkpoint listed first)', reqs[0].id === 'checkpoint-whyLocationMatters');
  for (const v of DIAGRAM_STAGE.requiredVariants) s = recordInteractionVariant(s, DIAGRAM_STAGE.id, v);
  const reqs2 = getReadinessRequirements(def, s);
  ok('the ESC-placement diagram requirement flips to met once satisfied, others remain unmet',
    reqs2.find(r => r.id === 'escPlacementDiagram')!.met && reqs2.filter(r => r.id !== 'escPlacementDiagram').every(r => !r.met));
}
...
console.log('\n[11] Backward navigation preserves already-recorded session progress');
{
  let s = createInitialSessionState(def);
  s = recordInteractionVariant(s, DIAGRAM_STAGE.id, 'esc');
  s = recordCheckpointAnswer(s, 'whyLocationMatters', CHECKPOINT_STAGES[0].checkpoint.options[0].id);
  s = goToStageId(def, s, 'escPlacementDiagram');
  s = goToStageId(def, s, 'orientation');
  ok('navigating backward does not clear a recorded checkpoint answer', isCheckpointAnswered(s, 'whyLocationMatters'));
  ok('navigating backward does not clear a recorded diagram variant', s.interactionVariants['escPlacementDiagram']?.['esc'] === true);
  s = goToStageId(def, s, 'escPlacementDiagram');
  ok('navigating forward again still preserves progress', s.interactionVariants['escPlacementDiagram']?.['esc'] === true);
  s = prevStage(def, s);
  ok('prevStage() by position also works and preserves progress', isCheckpointAnswered(s, 'whyLocationMatters'));
}
...
console.log('\n[12] A fresh session (equivalent to a page refresh) starts with zero interaction progress');
{
  ...
  ok('fresh state has no parts explored', Object.values(fresh.interactionVariants['escPlacementDiagram'] ?? {}).every(v => v === false));
...
console.log('\n[13] The comparison stage carries the cooled/secured/protected-vs-enclosed/loose contrast');
{
  ...
  ok('one item is the correct cooled/secured setup', labels.some(l => l.includes('الصحيح')));
  ok('one item is the unsafe enclosed/loose setup', labels.some(l => l.includes('غير الآمن')));
}

console.log('\n[14] Glossary covers the key ESC-placement vocabulary with real, non-empty MSA definitions');
{
  ok('exactly 5 glossary terms defined', GLOSSARY_STAGE.terms.length === 5);
  const termNames = GLOSSARY_STAGE.terms.map(t => t.term);
  ok('glossary defines proper ESC location', termNames.some(t => t.includes('موضع ESC')));
  ok('glossary defines heat/reliability', termNames.some(t => t.includes('الحرارة والموثوقية')));
  ok('glossary defines mechanical mounting', termNames.some(t => t.includes('التثبيت الميكانيكي')));
  ok('glossary defines conductive-carbon separation', termNames.some(t => t.includes('الفصل عن الكربون')));
  ok('glossary defines short-but-not-taut wiring', termNames.some(t => t.includes('أسلاك قصيرة')));
  ok('every glossary definition is substantive (non-empty)', GLOSSARY_STAGE.terms.every(t => t.definition.length > 10));
}

console.log('\n[15] Content-boundary check: Lesson 13 stays at ESC-placement level, not soldering/protocol/FC-install/flight depth');
{
  const allText = JSON.stringify(def).toLowerCase();
  const outOfScopeTerms = [
    'soldering', 'لحام', 'wire gauge', 'capacitor', 'dshot', 'betaflight', 'motor mixer',
    'motor 1', 'motor 2', 'motor 3', 'motor 4', 'current rating', 'amperage',
    'continuity', 'smoke stopper', 'grommet', 'fc installation', 'receiver installation', 'first flight',
  ];
  for (const term of outOfScopeTerms) {
    ok(`Lesson 13 does not mention out-of-scope term "${term}"`, !allText.includes(term.toLowerCase()));
  }
}

console.log('\n[16] Lesson 13\'s transition bridge to Lesson 14 is built from real lessonsData, not hardcoded text');
{
  ...
  if (completionStage.type === 'completion') {
    const bridgeText = completionStage.nextLessonBridge(lesson14);
    ok('bridge text contains Lesson 14\'s real title', bridgeText.includes(lesson14.title));
    ok('bridge text contains Lesson 14\'s real description', bridgeText.includes(lesson14.description));
  }
}

console.log('\n[17] Reused architecture: no Lesson-13-specific engine code exists — same generic engine as Lessons 1-12');
{
