...
async function main() {
  ...
  try {
    ...
    console.log('\n[1] Mobile navigation and the /community redirect');
    {
      ...
      // The SEQUENCE is `scripts/testNavOrder.ts`'s question, and the owner has
      // fixed a different one since this suite was written. What matters here is
      // what this suite is about: «البناء» is on the phone bar, reachable with a
      // thumb, and the retired community tab did not come back with the section.
      ok('«البناء» is on the phone bar', hrefs.includes('/build'));
      ...
      // NOT redirected. On the branch this section came from, /community was
      // retired and pointed at /build. Here it is only off the tab bar — the
      // route, its pages and its data are untouched by instruction, so the
      // honest assertion is the opposite one: it still answers for itself.
      const r = await fetch(`${BASE}/community`, { redirect: 'manual' });
      ok(`/community still answers for itself (${r.status})`, r.status === 200);
