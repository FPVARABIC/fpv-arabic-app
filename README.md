...
| `/search` | بحث شامل عبر كل محتوى التطبيق (عربي + إنجليزي + اختصارات + نقحرة + أخطاء إملائية) |
| `/glossary` | القاموس العربي التقني |
| `/diagnose` | التشخيص القائم على العرَض بأشجار متفرعة |

التصميم والتدقيق الكاملان في `docs/platform/`.
...
> سكربتا Playwright يبنيان حزمة إنتاج خاصة بهما في `dist-uitest` بقيم Firebase وهمية،
> لأن `src/lib/firebase.ts` يقرأ متغيرات البيئة عند تحميل الوحدة.
